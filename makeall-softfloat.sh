...
_make() {
	if [ -z "$1" ] ; then
		make CC="$CC" DESTDIR=${PREFIX} || exit 1
		#make DESTDIR=${PREFIX} || exit 1
		make CC="$CC" DESTDIR=${PREFIX} install || exit 1
...
_build() {
    ...
    SOFTSPECS=/usr/lib/gcc/i386-pc-fiwix/4.7.4/softfloat.specs
    export CC="gcc -specs=$SOFTSPECS"
   export CXX="g++ -specs=$SOFTSPECS"
		...
		newlib)
			...
			pushd $prg-$ver
				...
				./configure \
                    ...
                    CFLAGS_FOR_TARGET="-Os -msoft-float -mno-mmx -mno-sse -mno-sse2" \
                    CPPFLAGS_FOR_TARGET="-D_LDBL_EQ_DBL=1" || exit 1
...
# build libarchive 3.7.4 i386 "" "A library for handling streaming archive formats"
...
# build logrotate 3.22.0 i386 "" "Rotates, compresses, removes and mails system log files"
...
# build nano 8.1 i386 "" "A small text editor"
# build nasm 2.16.03 i386 "" "A portable x86 assembler which uses Intel-like syntax"
# build ncompress 5.0 i386 "" "Fast compression and decompression utilities"
# build ncurses-examples 20211021 i386 "" "A set of examples using ncurses"
...
# build tiff 4.5.1 i386 "" "Library of functions for manipulating TIFF format image files"
...
# build units 2.23 i386 "" "A utility for converting amounts from one unit to another"
...
# build vttest 20240218 i386 "" "Test VT100-type terminal"
